...
def load_model():
    try:
        return Llama(model_path=MODEL_PATH, n_gpu_layers=-1, n_ctx=2048, verbose=False)
...
def classify_user_intent(user_input):
    prompt = f"""[INST] <<SYS>>
あなたはテキスト分類アシスタントです。患者の最後の発言の意図を判断し、"question" (質問・懸念あり)か "end" (特にない・終了) のどちらかをJSON形式で出力してください。
<</SYS>>
# 患者の発言: {user_input} # JSON出力:[/INST]"""
    try:
        output = llm(prompt, max_tokens=32, stop=["}"], temperature=0.0)
...
def answer_general_question(user_question):
    prompt = f"""[INST] <<SYS>>
あなたはMRI検査に関する一般的な質問に答える、親切なAIアシスタントです。以下の質問に、簡潔かつ正確に答えてください。
<</SYS>>
# 質問: {user_question} # 回答:[/INST]"""
    try:
        output = llm(prompt, max_tokens=256, stop=["</s>"], temperature=0.7)
...
def move_to_next_question():
    ...
    while st.session_state.q_index < len(QUESTIONS):
        ...
        if should_skip:
            ...
            prev_q_info = QUESTIONS[current_q_index]
            if any(key in prev_q_info for key in ["follow_up_on_yes", "follow_up_on_no", "follow_up_on_detail"]):
                 message = f"次の質問です。\n\n{next_q_info['text']}"
            else:
    ...
    for q_info in QUESTIONS:
        ...
        if q_id in st.session_state.answers:
            ...
            if "detail" in answer_data:
                final_summary += f"\n> **詳細:** {answer_data['detail']}\n\n"
            elif "details" in answer_data:
                 final_summary += "\n> **追加の質問と懸念事項:**\n"
                 for item in answer_data.get('details', []):
...
def process_answer(user_input, **kwargs):
    ...
    if st.session_state.stage == "asking_detail":
        ...
        end_phrases = ["ありません", "ないです", "大丈夫です", "以上です", "特にありません", "ない"]
        ...
        if response_type == "dialogue":
            st.session_state.stage = "dialogue"; process_answer(user_input, **kwargs)
            return
        ...
        if response_type == "yes_no":
            ...
            if answer_type == "no":
                if current_q_info.get("end_if_no", False):
                    ...
                    # ★★★ follow_up_on_no のロジックをここに追加 ★★★
                    if "follow_up_on_no" in current_q_info:
                        st.session_state.messages.append({"role": "assistant", "content": current_q_info["follow_up_on_no"]})
                    move_to_next_question()
            elif answer_type == "yes":
                if "follow_up_on_yes" in current_q_info:
                    st.session_state.messages.append({"role": "assistant", "content": current_q_info["follow_up_on_yes"]})
                move_to_next_question()
            ...
            st.session_state.answers[q_id] = {"answer": user_input, "type": kwargs.get("answer_type", "unknown")}
            move_to_next_question()
...
if st.session_state.stage != "finished":
    ...
    if st.session_state.stage == "asking_detail":
        if user_input := st.chat_input("詳細を回答してください", key=input_key):
            process_answer(user_input); st.rerun()

    elif st.session_state.stage in ["asking_main", "dialogue"]:
        if response_type in ["yes_no", "yes_no_detail"]:
            col1, col2 = st.columns(2)
            if col1.button("はい", use_container_width=True, key=f"yes_{input_key}"):
                process_answer("はい"); st.rerun()
            if col2.button("いいえ", use_container_width=True, key=f"no_{input_key}"):
                process_answer("いいえ"); st.rerun()
        elif response_type == "exam_type":
            col1, col2, col3 = st.columns(3)
            if col1.button("造影検査", use_container_width=True, key=f"contrast_{input_key}"):
                process_answer("造影検査", answer_type="contrast"); st.rerun()
            if col2.button("単純検査", use_container_width=True, key=f"simple_{input_key}"):
                process_answer("単純検査", answer_type="simple"); st.rerun()
            if col3.button("わからない", use_container_width=True, key=f"unknown_{input_key}"):
                process_answer("わからない", answer_type="unknown"); st.rerun()
        elif response_type == "dialogue":
            if user_input := st.chat_input("質問がある場合は入力してください", key=input_key):
                process_answer(user_input); st.rerun()
            if st.button("特にありません（問診を終了）", use_container_width=True, key=f"end_dialogue_{input_key}"):
                process_answer("特にありません"); st.rerun()
else:
    st.success("問診はすべて終了しました。")